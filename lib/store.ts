...
export async function allPlaces():Promise<Place[]>{const r=await db().prepare("SELECT * FROM suggestions WHERE status='approved' ORDER BY created_at DESC").all<any>();return [...catalog,...r.results.map(p=>({id:p.id,city:p.city,category:p.category,name:p.name,area:p.city,address:p.address,description:p.note||'사용자 제안으로 등록된 장소입니다.',source:p.source,checked:new Date(p.decided_at).toISOString().slice(0,10),tags:['운영자 승인']}))];}
