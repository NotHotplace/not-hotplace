...
export function inRegion(place:{city:string;address?:string},region:string){return region==='전국'||(region==='청주'?(place.city==='청주'||/청주시/.test(place.address||'')):regionOf(place.city)===region);}
