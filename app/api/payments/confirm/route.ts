...
export async function POST(request:Request){
 ...
 if(order.status==='paid'){if(order.payment_key!==b.paymentKey)return Response.json({error:'결제 키가 주문과 달라요.'},{status:400});return Response.json({ok:true,until:order.access_until});}if(order.status!=='pending'||Date.now()-order.created_at>30*60000)return Response.json({error:'결제 요청이 만료되었어요.'},{status:409});
